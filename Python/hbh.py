...
try:
    ...
    res1 = requests.get(args.url, params=params1, allow_redirects=False, verify=not args.no_verify)
    if args.verbose:
        print('Trying %s?%s with hop-by-hop headers "%s"' % (args.url, params2['cb'], args.headers))
    res2 = requests.get(args.url, headers=headers, params=params2, allow_redirects=False, verify=not args.no_verify)
